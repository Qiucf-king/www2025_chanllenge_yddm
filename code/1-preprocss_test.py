...
def worker_fn(wid,data_lst_wids,):
    ...
    n_split = 10
    file_lst_wid = data_lst_wids[wid]
    file_lst_wids = chunk_list(file_lst_wid, len(file_lst_wid) // n_split, n_split)
    for sub_id, file_lst in enumerate(file_lst_wids):
        df = pd.DataFrame()
        for idx, file in enumerate(file_lst):
            df_ = make_df(root_dir, file)
            df = pd.concat([df, df_], axis=0)
            print('===== finished wid-{} part {}/{} sub {} / {} ====='.format(wid,idx+1,len(file_lst), sub_id+1, n_split ) )
        df.to_pickle('../data_tmp/test/part_{}_{}.pkl'.format(wid, sub_id) )
